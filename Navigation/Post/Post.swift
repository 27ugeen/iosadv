...
final class Post: NSObject, Shareable {
    let title: String
    let author: String
    let image: UIImage
    let descript: String
...
//                "title": "\(title)",
//                "author": "\(author)",
//                "image": "\\(String(describing: image.jpegData(compressionQuality: 1.0)!.base64EncodedString()))\",
//                "descript": "\\(descript)\",
...
//                """.data(using: .utf8)
    ...
    init(data: Data) {
        ...
        let likesStr = json["likes"] as! String
        likes = Int(likesStr)!
        let viewsStr = json["views"] as! String
        views = Int(viewsStr)!
...
// Providing Post to destinationApp
extension Post: NSItemProviderWriting {
    // Tell which identifiers are Post will be conforming to
    static var writableTypeIdentifiersForItemProvider: [String] {
        // General identifiers are not recommended, specific identifiers are welcome
        return [Post.shareIdentifier,
                UTType.text.identifier as String,
                UTType.text.identifier as String,
                UTType.image.identifier as String,
                UTType.text.identifier as String,
                kCIAttributeTypeInteger as String,
                kCIAttributeTypeInteger as String,
        ]
    }
    ...
    func loadData(withTypeIdentifier typeIdentifier: String, forItemProviderCompletionHandler completionHandler: @escaping (Data?, Error?) -> Void) -> Progress? {
        ...
        case Post.shareIdentifier:
            completionHandler(data, nil)
        case UTType.image.identifier as NSString as String:
            completionHandler(image.jpegData(compressionQuality: 1.0), nil)
        case UTType.text.identifier as NSString as String:
            completionHandler(title.data(using: .utf8), nil)
        case UTType.text.identifier as NSString as String:
            completionHandler(author.data(using: .utf8), nil)
        case UTType.text.identifier as NSString as String:
            completionHandler(descript.data(using: .utf8), nil)
        case kCIAttributeTypeInteger as NSString as String:
            completionHandler(String(describing: likes).data(using: .utf8), nil)
        case kCIAttributeTypeInteger as NSString as String:
            completionHandler(String(describing: views).data(using: .utf8), nil)
        default:
            completionHandler(nil, PostError.invalidTypeIdentifier)
        }
...
extension Post: NSItemProviderReading {
    ...
    static func object(withItemProviderData data: Data, typeIdentifier: String) throws -> Post {
        switch typeIdentifier {
        case Post.shareIdentifier:
            return Post(data: data)
        case UTType.image.identifier as NSString as String:
            return Post(title: "no title", author: "no author", image: UIImage(data: data)!, descript: "no descript", likes: 0, views: 0)
        case UTType.text.identifier as NSString as String:
            return Post(title: String(data: data, encoding: .utf8) ?? "",
                        author: String(data: data, encoding: .utf8) ?? "",
                        image: UIImage(),
                        descript: String(data: data, encoding: .utf8) ?? "",
                        likes: Int(String(data: data, encoding: .utf8) ?? "") ?? 0,
                        views: Int(String(data: data, encoding: .utf8) ?? "") ?? 0)
        default:
            throw PostError.invalidTypeIdentifier
        }
    }
}
